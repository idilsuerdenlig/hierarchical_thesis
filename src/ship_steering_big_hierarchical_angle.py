...
def server_experiment_small(alg_high, alg_low, params, experiment_params ,subdir, i):
    ...
    std1 = np.pi/6
    #sigma1 = Parameter(value=1)
    approximator1_mu = Regressor(LinearApproximator, input_shape=input_shape, output_shape=(1,))
    approximator1_std = Regressor(LinearApproximator, input_shape=input_shape, output_shape=(1,))
    approximator1_std.set_weights(np.ones(phi.size)*std1/n_tilings)
    pi1 = MultivariateStateStdGaussianPolicy(mu=approximator1_mu, std=approximator1_std)

    # Agent 1
    learning_rate1 = params.get('learning_rate_high')
    high = [1000, 1000, np.pi, np.pi / 12]
    ...
    agent1 = alg_high(policy=pi1, mdp_info=mdp_info_agent1, learning_rate=learning_rate1, features=phi)
...
if __name__ == '__main__':
    ...
    subdir = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '_big_hierarchical_angle/'
    alg_high = GPOMDP
    alg_low = GPOMDP
    #0.026179938779
