...
def experiment():
    ...
    tilingsH= Tiles.generate(n_tilings=1, n_tiles=[5, 5], low=[0,0], high=[lim, lim])
    ...
    # Agent1
    learning_rate1 = AdaptiveParameter(value=1e-8)
    agent1 = GhavamzadeAgent(pi1, mdp.info, learning_rate1, featuresL)

    # Agent2
    learning_rate2 = AdaptiveParameter(value=1e-8)
...
if __name__ == '__main__':
    subdir = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '_ghavamzade_small/'
