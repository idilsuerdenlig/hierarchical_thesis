...
class BonariniShipSteering(Environment):
    """
    ...
    def __init__(self, small=True, hard=False, n_steps_action=1):
        ...
        """
        self.__name__ = 'BonariniShipSteering'
        ...
        self._goal = np.aray([110, 110]) if small else np.array([910, 910])
        ...
        super(BonariniShipSteering, self).__init__(mdp_info)
        ...
        if new_state[0] > self.field_size or new_state[1] > self.field_size\
           or new_state[0] < 0 or new_state[1] < 0:
            ...
            if self.hard:
                ...
                del_x = self._goal[0] - new_state[0]
                del_y = self._goal[1] - new_state[1]
                theta_ref = normalize_angle(np.arctan2(del_y, del_x))
                reward = shortest_angular_distance(from_angle=new_state[2], to_angle=theta_ref)
            absorbing = False
